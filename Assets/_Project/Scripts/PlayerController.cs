    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        public void Interact()
        {
            ...
            if (Physics.Raycast(characterCamera.transform.position, characterCamera.transform.TransformDirection(Vector3.forward), out raycastHit, 5f))
            {
                ...
                /*if (targetObject.GetComponent<Item>())
                {
                    Debug.Log("Adding to inventory");
                    inventory.AddItemToInventoryRPC(targetObject);
                }
                else if(targetObject.GetComponent<Entrance>())
                {
                    targetObject.GetComponent<Entrance>().Interact(gameObject);
                }*/
            }
        }
